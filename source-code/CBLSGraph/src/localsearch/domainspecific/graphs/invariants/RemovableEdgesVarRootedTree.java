...
public class RemovableEdgesVarRootedTree implements GraphInvariant {
	...
	public String name(){ return "RemovableEdgesVarRootedTree";}
	@Override
	...
	public void initPropagate() {
		...
		for(Node v : vt.getNodes())if(v != vt.root() && vt.getAdj().get(v).size() == 1){
	...
	public void propagateAddEdge(VarRootedTree vt, Edge e) {
		...
		Node other = e.getEnd();
		Node _root = vt.root();
		//set{Edge}[] _adjEdges = vt.getAdjEdges();
		if(vt.isNull()){	
			removableEdges.add(e);
			return;
		}
...
/*		// TODO Auto-generated method stub
		...
		//if(_adjEdges[cv.getID()].getSize() != 1 && _adjEdges[fv.getID()].getSize() != 1){
	...
	public void propagateReplaceEdge(VarRootedTree vt, Edge eo, Edge ei) {
		...
		//set{Edge}[] _adjEdges = vt.getAdjEdges();
		if(u1 != u2){
			if(vt.getAdj().get(u1).size() == 1){
				for(Edge fe : vt.getAdj().get(u1)){
					removableEdges.remove(fe);
				}				
			}
			if(vt.getAdj().get(u2).size() == 2 && u2 != vt.root()){
				for(Edge ej : vt.getAdj().get(u2)){
			...
			if(vt.getAdj().get(u1).size() == 1 && u1 != vt.root()){
		...
		if(v1 != v2){
			if(vt.getAdj().get(v1).size() == 1){
				for(Edge fe : vt.getAdj().get(v1)){
					removableEdges.remove(fe);
				}
			}
			if(vt.getAdj().get(v2).size() == 2){
				for(Edge ej : vt.getAdj().get(v2)){
					Node other = ei.otherNode(v2);
					if(other != u2)
						removableEdges.add(ej);
				}
			}
